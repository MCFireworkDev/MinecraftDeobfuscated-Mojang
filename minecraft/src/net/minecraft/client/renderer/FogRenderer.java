	...
	public static void setupColor(Camera camera, float f, ClientLevel clientLevel, int i, float g) {
		...
		if (fluidState.is(FluidTags.WATER)) {
			...
			Vec3 vec33 = CubicSampler.gaussianSampleVec3(
				vec32, (ix, j, k) -> dimension.getBrightnessDependentFogColor(Vec3.fromRGB24(biomeManager.getNoiseBiomeAtQuart(ix, j, k).getFogColor()), y)
			);
			...
			fogBlue += (x - fogBlue) * u;
			float r = clientLevel.getRainLevel(f);
			if (r > 0.0F) {
				float s = 1.0F - r * 0.5F;
				float t = 1.0F - r * 0.4F;
				fogRed *= s;
