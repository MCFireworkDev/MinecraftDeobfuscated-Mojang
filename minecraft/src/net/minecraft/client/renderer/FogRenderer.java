	...
	public static void setupColor(Camera camera, float f, ClientLevel clientLevel, int i, float g) {
		...
		if (fogType == FogType.WATER) {
			if (entity instanceof LocalPlayer) {
				w = ((LocalPlayer)entity).getWaterVision();
			} else {
				w = 1.0F;
			}
		} else if (entity instanceof LivingEntity && ((LivingEntity)entity).hasEffect(MobEffects.NIGHT_VISION)) {
			w = GameRenderer.getNightVisionScale((LivingEntity)entity, f);
		} else {
			w = 0.0F;
		}

		if (fogRed != 0.0F && fogGreen != 0.0F && fogBlue != 0.0F) {
			float x = Math.min(1.0F / fogRed, Math.min(1.0F / fogGreen, 1.0F / fogBlue));
			fogRed = fogRed * (1.0F - w) + fogRed * x * w;
			fogGreen = fogGreen * (1.0F - w) + fogGreen * x * w;
			fogBlue = fogBlue * (1.0F - w) + fogBlue * x * w;
		}
