		...
		if (level == null) {
			...
			for(GameEventListenerRenderer.TrackedListener trackedListener : this.trackedListeners) {
				trackedListener.getPosition(level)
					.ifPresent(
						blockPosx -> {
							int ixx = blockPosx.getX() - trackedListener.getListenerRadius();
							int jxx = blockPosx.getY() - trackedListener.getListenerRadius();
							int kxx = blockPosx.getZ() - trackedListener.getListenerRadius();
							int lxx = blockPosx.getX() + trackedListener.getListenerRadius();
							int mxx = blockPosx.getY() + trackedListener.getListenerRadius();
							int n = blockPosx.getZ() + trackedListener.getListenerRadius();
							Vector3f vector3f = new Vector3f(1.0F, 1.0F, 0.0F);
							LevelRenderer.renderVoxelShape(
								poseStack,
								vertexConsumer,
								Shapes.create(new AABB((double)ixx, (double)jxx, (double)kxx, (double)lxx, (double)mxx, (double)n)),
								-d,
								-e,
								-f,
								vector3f.x(),
