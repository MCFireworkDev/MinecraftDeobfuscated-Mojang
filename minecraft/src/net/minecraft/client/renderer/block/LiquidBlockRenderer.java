...
public class LiquidBlockRenderer {
	...
	public boolean tesselate(BlockAndTintGetter blockAndTintGetter, BlockPos blockPos, VertexConsumer vertexConsumer, FluidState fluidState) {
		...
		if (!bl2 && !bl3 && !bl7 && !bl6 && !bl4 && !bl5) {
			...
			if (bl3) {
				float u = textureAtlasSprites[0].getU0();
				float w = textureAtlasSprites[0].getU1();
				float y = textureAtlasSprites[0].getV0();
				float aa = textureAtlasSprites[0].getV1();
			...
			for(int an = 0; an < 4; ++an) {
				...
				if (bl9 && !isFaceOccludedByNeighbor(blockAndTintGetter, blockPos, direction, Math.max(y, aa))) {
					bl8 = true;
					BlockPos blockPos2 = blockPos.relative(direction);
					...
					float as = textureAtlasSprite2.getV((double)((1.0F - y) * 16.0F * 0.5F));
					float at = textureAtlasSprite2.getV((double)((1.0F - aa) * 16.0F * 0.5F));
