	...
	public boolean tesselate(BlockAndTintGetter blockAndTintGetter, BlockPos blockPos, VertexConsumer vertexConsumer, FluidState fluidState) {
		...
		if (!bl2 && !bl3 && !bl7 && !bl6 && !bl4 && !bl5) {
			...
			for(int am = 0; am < 4; ++am) {
				...
				if (bl9 && !isFaceOccludedByNeighbor(blockAndTintGetter, blockPos, direction, Math.max(w, y))) {
					bl8 = true;
					BlockPos blockPos2 = blockPos.relative(direction);
					...
					float ak = textureAtlasSprite2.getV((double)((1.0F - w) * 16.0F * 0.5F));
					float ar = textureAtlasSprite2.getV((double)((1.0F - y) * 16.0F * 0.5F));
