	...
	public boolean tesselate(BlockAndBiomeGetter blockAndBiomeGetter, BlockPos blockPos, BufferBuilder bufferBuilder, FluidState fluidState) {
		...
		if (!bl2 && !bl3 && !bl7 && !bl6 && !bl4 && !bl5) {
			...
			if (bl3) {
				...
				float ap = 0.5F * g;
				float af = 0.5F * h;
				bufferBuilder.vertex(d, e, r + 1.0).color(aa, ap, af, 1.0F).uv((double)t, (double)z).uv2(an, ao).endVertex();
				bufferBuilder.vertex(d, e, r).color(aa, ap, af, 1.0F).uv((double)t, (double)x).uv2(an, ao).endVertex();
				bufferBuilder.vertex(d + 1.0, e, r).color(aa, ap, af, 1.0F).uv((double)v, (double)x).uv2(an, ao).endVertex();
				bufferBuilder.vertex(d + 1.0, e, r + 1.0).color(aa, ap, af, 1.0F).uv((double)v, (double)z).uv2(an, ao).endVertex();
				bl8 = true;
			}
			...
			for(int aq = 0; aq < 4; ++aq) {
				...
				if (bl9 && !isFaceOccluded(blockAndBiomeGetter, blockPos, direction, Math.max(v, x))) {
					...
					float bc = 1.0F * ba * g;
					float bd = 1.0F * ba * h;
					bufferBuilder.vertex(ar, e + (double)v, at).color(bb, bc, bd, 1.0F).uv((double)av, (double)aj).uv2(ay, az).endVertex();
					bufferBuilder.vertex(as, e + (double)x, au).color(bb, bc, bd, 1.0F).uv((double)aw, (double)ak).uv2(ay, az).endVertex();
