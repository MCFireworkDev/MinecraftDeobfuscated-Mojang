...
public class BiomeColors {
	...
	private static int getAverageColor(BlockAndBiomeGetter blockAndBiomeGetter, BlockPos blockPos, BiomeColors.ColorResolver colorResolver) {
		...
		if (l == 0) {
			...
			int n;
			for(BlockPos.MutableBlockPos mutableBlockPos = new BlockPos.MutableBlockPos(); cursor3D.advance(); k += n & 0xFF) {
				mutableBlockPos.set(cursor3D.nextX(), cursor3D.nextY(), cursor3D.nextZ());
				n = colorResolver.getColor(blockAndBiomeGetter.getBiome(mutableBlockPos), mutableBlockPos);
				i += (n & 0xFF0000) >> 16;
				j += (n & 0xFF00) >> 8;
			}
