...
public class BannerRenderer extends BlockEntityRenderer<BannerBlockEntity> {
	...
	public void render(
		BannerBlockEntity bannerBlockEntity, double d, double e, double f, float g, PoseStack poseStack, MultiBufferSource multiBufferSource, int i
	) {
		float h = 0.6666667F;
		...
		if (bl) {
			...
			if (blockState.getBlock() instanceof BannerBlock) {
				...
				poseStack.mulPose(Vector3f.YP.rotation((float)(-blockState.getValue(BannerBlock.ROTATION) * 360) / 16.0F, true));
