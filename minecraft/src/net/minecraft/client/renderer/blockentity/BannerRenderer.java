...
public class BannerRenderer implements BlockEntityRenderer<BannerBlockEntity> {
		...
		if (list != null) {
			...
			if (bl) {
				...
				if (blockState.getBlock() instanceof BannerBlock) {
					poseStack.translate(0.5, 0.5, 0.5);
					float h = (float)(-blockState.getValue(BannerBlock.ROTATION) * 360) / 16.0F;
	...
	public static void renderPatterns(
		...
		for(int k = 0; k < 17 && k < list.size(); ++k) {
			...
			Material material2 = new Material(bl ? Sheets.BANNER_SHEET : Sheets.SHIELD_SHEET, ((BannerPattern)pair.getFirst()).location(bl));
