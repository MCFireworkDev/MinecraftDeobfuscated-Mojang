...
public class SectionOcclusionGraph {
	...
	private void queueSectionsWithNewNeighbors(SectionOcclusionGraph.GraphState graphState) {
		...
		while(longIterator.hasNext()) {
			...
			List<SectionRenderDispatcher.RenderSection> list = (List)graphState.storage.chunksWaitingForNeighbors.get(l);
				...
				if (renderSection2 != null && (!bl || !node.hasDirection(direction.getOpposite()))) {
					...
					if (node2 != null) {
						...
						if (renderSection2.hasAllNeighbors()) {
							...
							graphStorage.sectionToNodeMap.put(renderSection2, node3);
							((List)graphStorage.chunksWaitingForNeighbors.computeIfAbsent(ChunkPos.asLong(renderSection2.getOrigin()), (Long2ObjectFunction)(l -> new ArrayList())))
								.add(renderSection2);
						}
					}
				}
	...
	protected static class Node {
		...
		public boolean equals(Object object) {
			if (!(object instanceof SectionOcclusionGraph.Node)) {
				return false;
			} else {
				SectionOcclusionGraph.Node node = (SectionOcclusionGraph.Node)object;
				return this.section.getOrigin().equals(node.section.getOrigin());
			}
		}
	}
