...
public class ChatReportBuilder {
		...
		this.reportedMessages.forEach(i -> {
			...
			for(UUID uUID : set) {
				this.chainForPlayer(chatLog, int2ObjectMap, uUID).forEach(entry -> {
					...
					if (loggedChatMessageLink instanceof LoggedChatMessage.Player playerxx) {
						int2ObjectSortedMap.putIfAbsent(entry.id(), this.buildReportedChatMessage(entry.id(), playerxx));
					} else {
	...
	private static void walkMessageReferenceGraph(ChatLog chatLog, int i, ChatReportBuilder.ReferencedMessageVisitor referencedMessageVisitor) {
		...
		while(!intPriorityQueue.isEmpty()) {
			...
			LoggedChatEvent var7 = chatLog.lookup(j);
			if (var7 instanceof LoggedChatMessage.Player player) {
	...
	private static IntCollection messageReferences(ChatLog chatLog, int i, PlayerChatMessage playerChatMessage) {
		Set<MessageSignature> set = (Set)playerChatMessage.signedBody()
			.lastSeen()
			.entries()
			.stream()
		...
		while(iterator.hasNext() && !set.isEmpty()) {
			ChatLog.Entry<LoggedChatEvent> entry = (ChatLog.Entry)iterator.next();
			LoggedChatEvent var9 = entry.event();
			if (var9 instanceof LoggedChatMessage.Player player && set.remove(player.headerSignature())) {
