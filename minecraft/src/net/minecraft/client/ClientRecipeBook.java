...
public class ClientRecipeBook extends RecipeBook {
	...
	public void setupCollections(Iterable<Recipe<?>> iterable) {
		...
		map.forEach((recipeBookCategories, list) -> list.stream().map(RecipeCollection::new).peek(builder::add));
		RecipeBookCategories.AGGREGATE_CATEGORIES
			.forEach(
				(recipeBookCategories, list) -> list.stream()
	...
	private static Map<RecipeBookCategories, List<List<Recipe<?>>>> categorizeAndGroupRecipes(Iterable<Recipe<?>> iterable) {
		...
		for(Recipe<?> recipe : iterable) {
			if (!recipe.isSpecial() && !recipe.isIncomplete()) {
				...
				if (string.isEmpty()) {
					((List)map.computeIfAbsent(recipeBookCategories, recipeBookCategoriesx -> Lists.newArrayList())).add(ImmutableList.<Recipe<?>>of(recipe));
				} else {
					List<Recipe<?>> list = (List)table.get(recipeBookCategories, string);
					if (list == null) {
