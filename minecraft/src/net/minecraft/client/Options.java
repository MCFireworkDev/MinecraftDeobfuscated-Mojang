...
public class Options {
			...
			try {
				bufferedReader.lines().forEach(stringx -> {
					try {
						...
						System.gc();
						throw new Options.OptionParseError("Failed to parse option: " + stringx.substring(0, Math.min(200, stringx.length())), var3xx);
	...
	public float getSoundSourceVolume(SoundSource soundSource) {
		return this.sourceVolumes.containsKey(soundSource) ? this.sourceVolumes.get(soundSource) : 1.0F;
	}
