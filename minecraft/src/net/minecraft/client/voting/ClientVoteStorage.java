...
public class ClientVoteStorage {
	...
	public void startVote(UUID uUID, ClientVote clientVote) {
		...
		List<List<ClientVoteStorage.StyledCodepoint>> list = (List)clientVote.options()
			.values()
			.stream()
			.filter(clientOption -> !clientOption.irregular())
			.map(
				clientOption -> {
					List<ClientVoteStorage.StyledCodepoint> listxx = new ArrayList();
	...
	public static class VoteInfo {
		...
		public void storeVotes(int i, OptionVotes optionVotes) {
			((Map)this.optionToVoters.computeIfAbsent(i, (Int2ObjectFunction)(ix -> new HashMap()))).putAll(optionVotes.voters());
		}
		...
		private int voteCount(UUID uUID, OptionId optionId) {
			Map<UUID, Voter> map = (Map)this.optionToVoters.get(optionId.index());
			if (map == null) {
				return 0;
			} else {
