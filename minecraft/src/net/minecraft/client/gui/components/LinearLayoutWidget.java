...
public class LinearLayoutWidget extends AbstractContainerWidget {
	...
	private final List<AbstractWidget> containedChildrenView = Collections.unmodifiableList(
		Lists.transform(this.children, childContainer -> childContainer.child)
	);
	...
	public void pack() {
		if (!this.children.isEmpty()) {
			...
			LinearLayoutWidget.ChildContainer childContainer3;
			if (this.children.size() >= 2) {
				for(Divisor divisor = new Divisor(k, this.children.size() - 1); divisor.hasNext(); l += this.orientation.getPrimaryLength(childContainer3)) {
					l += divisor.nextInt();
					childContainer3 = (LinearLayoutWidget.ChildContainer)iterator.next();
