...
public class ChatComponent extends GuiComponent {
		...
		if (!this.isChatHidden()) {
			...
			if (k > 0) {
				...
				for(int n = 0; n + this.chatScrollbarPos < this.trimmedMessages.size() && n < j; ++n) {
					GuiMessage<FormattedCharSequence> guiMessage = (GuiMessage)this.trimmedMessages.get(n + this.chatScrollbarPos);
					if (guiMessage != null) {
				...
				if (!this.chatQueue.isEmpty()) {
					...
					RenderSystem.enableBlend();
					poseStack.translate(0.0, 0.0, 50.0);
					this.minecraft.font.drawShadow(poseStack, new TranslatableComponent("chat.queue", this.chatQueue.size()), 0.0F, 1.0F, 16777215 + (n << 24));
					poseStack.popPose();
					RenderSystem.disableBlend();
				}
	...
	public void rescaleChat() {
		...
		for(int i = this.allMessages.size() - 1; i >= 0; --i) {
			GuiMessage<Component> guiMessage = (GuiMessage)this.allMessages.get(i);
			...
			if (!(f < 0.0) && !(g < 0.0)) {
				...
				if (f <= (double)Mth.floor((double)this.getWidth() / this.getScale()) && g < (double)(9 * i + i)) {
					...
					if (j >= 0 && j < this.trimmedMessages.size()) {
						GuiMessage<FormattedCharSequence> guiMessage = (GuiMessage)this.trimmedMessages.get(j);
