		...
		if (!this.isChatHidden()) {
			...
			if (k > 0) {
				...
				for(int o = 0; o + this.chatScrollbarPos < this.trimmedMessages.size() && o < j; ++o) {
					GuiMessage<FormattedCharSequence> guiMessage = (GuiMessage)this.trimmedMessages.get(o + this.chatScrollbarPos);
					if (guiMessage != null) {
				...
				if (!this.chatQueue.isEmpty()) {
					...
					RenderSystem.enableBlend();
					poseStack.translate(0.0, 0.0, 50.0);
					this.minecraft.font.drawShadow(poseStack, new TranslatableComponent("chat.queue", this.chatQueue.size()), 0.0F, 1.0F, 16777215 + (o << 24));
					poseStack.popPose();
					RenderSystem.disableBlend();
				}
