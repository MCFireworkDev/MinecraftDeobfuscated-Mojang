				...
				public PackSelectionModel<?> apply(Runnable runnable) {
					...
					return new PackSelectionModel(
						runnable, (pack, textureManager) -> textureManager.bind(DataPackSelectScreen.DEFAULT_ICON), Lists.reverse(list2), list3, (listx, list2x, bl) -> {
							List<String> list3xx = (List)Lists.reverse(listx).stream().map(Pack::getId).collect(ImmutableList.toImmutableList());
							List<String> list4 = (List)list2x.stream().map(Pack::getId).collect(ImmutableList.toImmutableList());
							this.workingConfig = new DataPackConfig(list3xx, list4);
							if (!bl) {
								this.repository.setSelected(list3xx);
