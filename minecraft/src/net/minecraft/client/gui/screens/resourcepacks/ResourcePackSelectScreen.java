			...
			if (this.changed) {
				...
				for(UnopenedResourcePack unopenedResourcePackxx : listxx) {
					if (!unopenedResourcePackxx.isFixedPosition()) {
						this.minecraft.options.resourcePacks.add(unopenedResourcePackxx.getId());
						if (!unopenedResourcePackxx.getCompatibility().isCompatible()) {
							this.minecraft.options.incompatibleResourcePacks.add(unopenedResourcePackxx.getId());
						}
					}
				}
