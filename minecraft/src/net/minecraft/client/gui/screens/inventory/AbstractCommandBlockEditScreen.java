		...
		public void render(int i, int j) {
			...
			if (bl3) {
				...
				if (bl2) {
					for(int n = 0; n < this.rect.getWidth(); ++n) {
						if (n % 2 == 0) {
							GuiComponent.fill(
								this.rect.getX() + n, this.rect.getY() + this.rect.getHeight(), this.rect.getX() + n + 1, this.rect.getY() + this.rect.getHeight() + 1, -1
							);
						}
					}
				}
			...
			for(int o = 0; o < k; ++o) {
				...
				AbstractCommandBlockEditScreen.this.font
					.drawShadow(
						suggestion.getText(), (float)(this.rect.getX() + 1), (float)(this.rect.getY() + 2 + 12 * o), o + this.offset == this.current ? -256 : -5592406
					);
			}
