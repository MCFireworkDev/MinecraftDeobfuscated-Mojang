...
public class Font {
		...
		for(int m = -1; m <= 1; ++m) {
			for(int n = -1; n <= 1; ++n) {
				if (m != 0 || n != 0) {
					...
					int o = m;
					int p = n;
					formattedCharSequence.accept((lx, style, m) -> {
						...
						return stringRenderOutput.accept(lx, style.withColor(l), m);
					});
				}
			}
		}
	...
	private float renderText(
		String string, float f, float g, int i, boolean bl, Matrix4f matrix4f, MultiBufferSource multiBufferSource, boolean bl2, int j, int k
	) {
