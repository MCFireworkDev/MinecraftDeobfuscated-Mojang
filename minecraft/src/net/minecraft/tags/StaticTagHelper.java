...
public class StaticTagHelper<T> {
	...
	public void reset(TagContainer tagContainer) {
		TagCollection<T> tagCollection = (TagCollection)this.collectionGetter.apply(tagContainer);
	...
	public Set<ResourceLocation> getMissingTags(TagContainer tagContainer) {
		TagCollection<T> tagCollection = (TagCollection)this.collectionGetter.apply(tagContainer);
		Set<ResourceLocation> set = (Set)this.wrappers.stream().map(StaticTagHelper.Wrapper::getName).collect(Collectors.toSet());
