...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider {
	...
	private static boolean isChunkOnRangeBorder(int i, int j, int k, int l, int m) {
		if (!isChunkInRange(i, j, k, l, m)) {
			...
			return !isChunkInRange(i + 1, j + 1, k, l, m)
		...
		if (Math.abs(k - i) <= o * 2 && Math.abs(n - j) <= o * 2) {
			...
			for(int p = i - o; p <= i + o; ++p) {
				for(int q = j - o; q <= j + o; ++q) {
					if (isChunkInRange(p, q, i, j, this.viewDistance)) {
						boolean bl6 = false;
						boolean bl7 = true;
						this.updateChunkTracking(serverPlayer, new ChunkPos(p, q), new MutableObject<>(), false, true);
					}
				}
			}
		}
