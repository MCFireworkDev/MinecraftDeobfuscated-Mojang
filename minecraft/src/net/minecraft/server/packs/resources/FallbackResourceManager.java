...
public class FallbackResourceManager implements ResourceManager {
	...
	public Map<ResourceLocation, ResourceThunk> listResources(String string, Predicate<ResourceLocation> predicate) {
		...
		for(Entry<ResourceLocation> entry : Object2IntMaps.fastIterable(object2IntMap)) {
			...
			map.put(resourceLocation2, new ResourceThunk(string2, () -> {
				ResourceLocation resourceLocation2xx = getMetadataLocation(resourceLocation2);
				InputStream inputStream = null;

				for(int j = this.fallbacks.size() - 1; j >= k; --j) {
					FallbackResourceManager.PackEntry packEntryxx = (FallbackResourceManager.PackEntry)this.fallbacks.get(j);
					PackResources packResources2 = packEntryxx.resources;
					if (packResources2 != null && packResources2.hasResource(this.type, resourceLocation2xx)) {
						inputStream = this.getWrappedResource(resourceLocation2xx, packResources2);
						break;
					}

					if (packEntryxx.isFiltered(resourceLocation2xx)) {
						break;
					}
				}
	...
	private static void applyPackFiltersToExistingResources(
		...
		while(iterator.hasNext()) {
			java.util.Map.Entry<ResourceLocation, FallbackResourceManager.EntryStack> entry = (java.util.Map.Entry)iterator.next();
