...
class LinkFSPath implements Path {
	...
	public File toFile() {
		...
		if (var2 instanceof PathContents.FileContents fileContents) {
	...
	LinkFSPath resolveName(String string) {
		if (isRelativeOrMissing(this.pathContents)) {
			...
			if (linkFSPath instanceof PathContents.DirectoryContents directoryContents) {
				LinkFSPath linkFSPathx = (LinkFSPath)directoryContents.children().get(string);
				return linkFSPathx != null ? linkFSPathx : new LinkFSPath(this.fileSystem, string, this, PathContents.MISSING);
	...
	public Path getTargetPath() {
		PathContents var2 = this.pathContents;
		return var2 instanceof PathContents.FileContents fileContents ? fileContents.contents() : null;
	}

	@Nullable
	public PathContents.DirectoryContents getDirectoryContents() {
		PathContents var2 = this.pathContents;
		return var2 instanceof PathContents.DirectoryContents directoryContents ? directoryContents : null;
	}
