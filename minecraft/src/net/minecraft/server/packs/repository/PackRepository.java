...
public class PackRepository<T extends UnopenedPack> implements AutoCloseable {
	...
	public void reload() {
		List<String> list = (List)this.selected.stream().map(UnopenedPack::getId).collect(ImmutableList.toImmutableList());
		this.close();
	...
	private List<T> rebuildSelected(Collection<String> collection) {
		List<T> list = (List)this.getAvailablePacks(collection).collect(Collectors.toList());
