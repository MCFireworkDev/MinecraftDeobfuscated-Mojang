...
public record AdvancementRequirements(String[][] requirements) {
	...
	public static AdvancementRequirements allOf(Collection<String> collection) {
		return new AdvancementRequirements((String[][])collection.stream().map(string -> new String[]{string}).toArray(i -> new String[i][]));
	}

	public static AdvancementRequirements anyOf(Collection<String> collection) {
		return new AdvancementRequirements(new String[][]{(String[])collection.toArray(i -> new String[i])});
	}
	...
	public static AdvancementRequirements fromJson(JsonArray jsonArray, Set<String> set) {
		...
		if (!set.equals(set2)) {
			Set<String> set3 = Sets.difference(set, set2);
			Set<String> set4 = Sets.difference(set2, set);
