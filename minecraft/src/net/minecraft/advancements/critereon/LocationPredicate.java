...
public class LocationPredicate {
	...
	public boolean matches(ServerLevel serverLevel, float f, float g, float h) {
		if (!this.x.matches(f)) {
			...
			Optional<ResourceKey<Biome>> optional = serverLevel.registryAccess()
	...
	public static LocationPredicate fromJson(@Nullable JsonElement jsonElement) {
		if (jsonElement != null && !jsonElement.isJsonNull()) {
			...
			return new LocationPredicate(
				floats, floats2, floats3, resourceKey2, structureFeature, resourceKey, boolean_, lightPredicate, blockPredicate, fluidPredicate
			);
		} else {
			return ANY;
		}
