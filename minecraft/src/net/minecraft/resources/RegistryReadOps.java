...
public class RegistryReadOps<T> extends DelegatingOps<T> {
	...
	private <E> DataResult<Supplier<E>> readAndRegisterElement(
		...
		DataResult<Supplier<E>> dataResult = (DataResult)readCache.values.get(resourceKey2);
		if (dataResult != null) {
			return dataResult;
		} else {
			...
			if (optional.isPresent()) {
				...
				writableRegistry.registerOrOverride((OptionalInt)pair.getSecond(), resourceKey2, pair.getFirst(), dataResult2.lifecycle());
			}

			DataResult<Supplier<E>> dataResult3;
			if (!optional.isPresent() && writableRegistry.get(resourceKey2) != null) {
				dataResult3 = DataResult.success((Supplier)() -> writableRegistry.get(resourceKey2), Lifecycle.stable());
			} else {
