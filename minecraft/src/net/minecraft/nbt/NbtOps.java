	...
	public DataResult<Tag> mergeToList(Tag tag, List<Tag> list) {
		if (!(tag instanceof CollectionTag) && !(tag instanceof EndTag)) {
			...
			CollectionTag<?> collectionTag = createGenericList(
				tag instanceof CollectionTag ? ((CollectionTag)tag).getElementType() : 0, list.stream().findFirst().map(Tag::getId).orElse((byte)0)
			);
		...
		if (!(tag instanceof CompoundTag) && !(tag instanceof EndTag)) {
			...
			mapLike.entries().forEach(pair -> {
				...
				if (!(tagxx instanceof StringTag)) {
					...
					compoundTag.put(tagxx.getAsString(), (Tag)pair.getSecond());
				}
			});
	...
	public DataResult<Consumer<BiConsumer<Tag, Tag>>> getMapEntries(Tag tag) {
		if (!(tag instanceof CompoundTag)) {
			...
			return DataResult.success(
				(Consumer)biConsumer -> compoundTag.getAllKeys().forEach(string -> biConsumer.accept(this.createString(string), compoundTag.get(string)))
			);
		}
	}
	...
	public Tag createList(Stream<Tag> stream) {
		...
		if (!peekingIterator.hasNext()) {
			...
			if (tag instanceof ByteTag) {
				List<Byte> list = Lists.newArrayList(Iterators.transform(peekingIterator, tagx -> ((ByteTag)tagx).getAsByte()));
				return new ByteArrayTag(list);
			} else if (tag instanceof IntTag) {
				List<Integer> list = Lists.newArrayList(Iterators.transform(peekingIterator, tagx -> ((IntTag)tagx).getAsInt()));
				return new IntArrayTag(list);
			} else if (tag instanceof LongTag) {
				List<Long> list = Lists.newArrayList(Iterators.transform(peekingIterator, tagx -> ((LongTag)tagx).getAsLong()));
