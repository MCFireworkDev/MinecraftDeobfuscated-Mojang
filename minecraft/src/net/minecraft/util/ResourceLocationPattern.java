...
public class ResourceLocationPattern {
	...
	private ResourceLocationPattern(Optional<Pattern> optional, Optional<Pattern> optional2) {
		...
		this.namespacePredicate = (Predicate)optional.map(Pattern::asPredicate).orElse((Predicate)string -> true);
		this.pathPattern = optional2;
		this.pathPredicate = (Predicate)optional2.map(Pattern::asPredicate).orElse((Predicate)string -> true);
