...
public interface PreviewedArgument<T> extends ArgumentType<T> {
	...
	static CompletableFuture<Component> resolvePreviewed(
		...
		ArgumentType var3 = argumentCommandNode.getType();
		return var3 instanceof PreviewedArgument previewedArgument ? previewedArgument.resolvePreview(commandContextBuilder, argumentCommandNode.getName()) : null;
	}
	...
	default CompletableFuture<Component> resolvePreview(CommandContextBuilder<CommandSourceStack> commandContextBuilder, String string) throws CommandSyntaxException {
		ParsedArgument<CommandSourceStack, ?> parsedArgument = (ParsedArgument)commandContextBuilder.getArguments().get(string);
