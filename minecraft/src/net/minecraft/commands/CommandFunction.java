...
public class CommandFunction {
	...
	public static CommandFunction fromLines(
		...
		return (CommandFunction)(set.isEmpty()
			? new CommandFunction(resourceLocation, (CommandFunction.Entry[])list2.toArray(ix -> new CommandFunction.Entry[ix]))
			: new CommandFunction.CommandMacro(resourceLocation, (CommandFunction.Entry[])list2.toArray(ix -> new CommandFunction.Entry[ix]), List.copyOf(set)));
	}
	...
	static class CommandMacro extends CommandFunction {
		...
		private CommandFunction substituteAndParse(List<String> list, CommandDispatcher<CommandSourceStack> commandDispatcher, CommandSourceStack commandSourceStack) throws FunctionInstantiationException {
			...
			for(int i = 0; i < entrys.length; ++i) {
				...
				if (!(entry instanceof CommandFunction.MacroEntry)) {
					entrys2[i] = entry;
				} else {
					CommandFunction.MacroEntry macroEntry = (CommandFunction.MacroEntry)entry;
