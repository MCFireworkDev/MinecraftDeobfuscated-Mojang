				...
				for(int i = executionContext.forkLimit(); contextChain.getStage() != Stage.EXECUTE; contextChain = contextChain.nextStage()) {
					...
					if (redirectModifier != null) {
						...
						for(T executionCommandSource2 : list2) {
							try {
								Collection<T> collection = ContextChain.runModifier(commandContext, executionCommandSource2, (commandContext, bl, i) -> {
								}, (boolean)customModifierExecutor);
								if (list3.size() + collection.size() >= i) {
									executionCommandSource.handleError(ERROR_FORK_LIMIT_REACHED.create(i), (boolean)customModifierExecutor, executionContext.tracer());
									return;
								}

								list3.addAll(collection);
							} catch (CommandSyntaxException var20) {
								executionCommandSource2.handleError(var20, (boolean)customModifierExecutor, executionContext.tracer());
