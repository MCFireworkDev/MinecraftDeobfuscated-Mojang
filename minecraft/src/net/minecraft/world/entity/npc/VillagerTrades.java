			...
			hashMap.put(
				...
				toIntMap(
					ImmutableMap.of(
						...
						5,
						new VillagerTrades.ItemListing[]{
							new VillagerTrades.ItemsForEmeralds(Blocks.DIAMOND_BLOCK, 1, 120),
							new VillagerTrades.EmeraldForItems(CarryableTrade.entity(EntityType.CREEPER), 1, 120)
						}
					)
				)
			);
		...
		public ItemsForEmeralds(Item item, int i, int j) {
			this(
				CarryableTrade.block(
					(Block)CarriedBlocks.getBlockFromItemStack(item.getDefaultInstance())
