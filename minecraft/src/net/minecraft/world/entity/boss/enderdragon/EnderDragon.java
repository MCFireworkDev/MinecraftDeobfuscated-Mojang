			...
			if (this.isNoAi()) {
				...
				if (this.level.isClientSide) {
					...
					if (optional.isPresent()) {
						...
						if (var38 instanceof Player player) {
					...
					if (vec32 != null) {
						...
						if (Math.abs(l) > 1.0E-5F || Math.abs(n) > 1.0E-5F) {
							...
							this.yRotA *= 0.8F;
							this.yRotA += s * dragonPhaseInstance.getTurnSpeed();
				...
				for(int af = 0; af < 3; ++af) {
					...
					this.tickPart(enderDragonPart, (double)(-(aa * 1.5F + ah * t) * x), es[1] - ds[1] - (double)((t + 1.5F) * y) + 1.5, (double)((ab * 1.5F + r * t) * x));
				}
