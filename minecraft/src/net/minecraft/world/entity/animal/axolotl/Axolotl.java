...
public class Axolotl extends Animal implements Bucketable {
	...
	public InteractionResult mobInteract(Player player, InteractionHand interactionHand) {
		return (InteractionResult)ItemUtils.bucketMobPickup(
				player, interactionHand, this, SoundEvents.BUCKET_FILL_AXOLOTL, () -> new ItemStack(Items.AXOLOTL_BUCKET)
			)
	...
	public static enum Variant {
		...
		private static Axolotl.Variant getSpawnVariant(Random random, boolean bl) {
			Axolotl.Variant[] variants = (Axolotl.Variant[])Arrays.stream(BY_ID).filter(variant -> variant.common == bl).toArray(i -> new Axolotl.Variant[i]);
			return Util.getRandom((Axolotl.Variant[])variants, random);
		}
	}
}
