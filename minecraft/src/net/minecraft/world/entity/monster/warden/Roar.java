...
public class Roar extends Behavior<Warden> {
	...
	protected void tick(ServerLevel serverLevel, Warden warden, long l) {
		...
		optional.ifPresent(long_ -> {
			Optional<Long> optionalxx = warden.getBrain().getMemory(MemoryModuleType.LAST_AUDIBLE_ROAR);
			boolean bl = optionalxx.isEmpty() || l > optionalxx.get() + (long)WardenAi.ROAR_DURATION;
			if (bl && l >= 25L + long_) {
