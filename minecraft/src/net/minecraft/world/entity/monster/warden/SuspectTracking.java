...
public class SuspectTracking {
	private final Map<UUID, Integer> suspectIdToAnger = Maps.newHashMap();
	...
	public void update() {
		if (this.shouldDecrement.get()) {
			this.suspectIdToAnger.replaceAll((uUID, integer) -> Math.max(0, integer - this.decrementAngerBy));
			this.suspectIdToAnger.entrySet().removeIf(entry -> entry.getValue() <= 0);
		}
	}
	...
	public int getAngerFor(UUID uUID) {
		return !this.suspectIdToAnger.containsKey(uUID) ? 0 : this.suspectIdToAnger.get(uUID);
	}

	public int getActiveAnger() {
		return this.getTopEntry().map(Entry::getValue).orElse(0);
	}
