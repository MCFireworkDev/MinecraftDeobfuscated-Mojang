...
public class TryToSniff extends Behavior<Warden> {
	...
	protected boolean checkExtraStartConditions(ServerLevel serverLevel, Warden warden) {
		...
		boolean bl = optional.map(long_ -> serverLevel.getGameTime() - long_ >= 100L).orElse(true);
		if (bl) {
			if (brain.hasMemoryValue(MemoryModuleType.LAST_SNIFF)) {
				long l = serverLevel.getGameTime() - brain.getMemory(MemoryModuleType.LAST_SNIFF).get();
				return l >= 120L;
			} else {
				return true;
			}
