...
public class SetClosestHomeAsWalkTarget extends Behavior<LivingEntity> {
	...
	protected boolean checkExtraStartConditions(ServerLevel serverLevel, LivingEntity livingEntity) {
		if (serverLevel.getGameTime() - this.lastUpdate < 20L) {
			...
			Optional<BlockPos> optional = poiManager.findClosest(
				PoiType.HOME.getPredicate(), blockPos -> true, new BlockPos(livingEntity), 48, PoiManager.Occupancy.ANY
			);
	...
	protected void start(ServerLevel serverLevel, LivingEntity livingEntity, long l) {
		...
		Predicate<BlockPos> predicate = blockPosx -> {
			...
			if (this.batchCache.containsKey(lxx)) {
				return false;
			} else if (++this.triedCount >= 5) {
				return false;
			} else {
				this.batchCache.put(lxx, this.lastUpdate + 40L);
				return true;
			}
		};
