...
public class InteractWithDoor extends Behavior<LivingEntity> {
	...
	protected void start(ServerLevel serverLevel, LivingEntity livingEntity, long l) {
		...
		List<GlobalPos> list = (List)brain.getMemory(MemoryModuleType.INTERACTABLE_DOORS).get();
		List<BlockPos> list2 = (List)path.getNodes().stream().map(node -> new BlockPos(node.x, node.y, node.z)).collect(Collectors.toList());
