...
public class LongJumpToRandomPos<E extends Mob> extends Behavior<E> {
	...
	protected boolean isAcceptableLandingPosition(ServerLevel serverLevel, E mob, BlockPos blockPos) {
		...
		if (i == blockPos.getX() && j == blockPos.getZ()) {
			return false;
		} else if (!mob.getNavigation().isStableDestination(blockPos) && !this.acceptableLandingSpot.test(serverLevel.getBlockState(blockPos.below()))) {
			return false;
		} else {
			return mob.getPathfindingMalus(WalkNodeEvaluator.getBlockPathTypeStatic(mob.level, blockPos.mutable())) == 0.0F;
		}
	}
