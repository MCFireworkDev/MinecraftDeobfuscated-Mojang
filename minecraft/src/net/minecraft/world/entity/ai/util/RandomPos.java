	...
	private static Vec3 generateRandomPos(
		...
		for(int n = 0; n < 10; ++n) {
			...
			if (blockPos2 != null) {
				...
				BlockPos blockPos3 = new BlockPos((double)o + pathfinderMob.getX(), (double)p + pathfinderMob.getY(), (double)q + pathfinderMob.getZ());
				if ((!bl4 || pathfinderMob.isWithinRestriction(blockPos3)) && (!bl3 || pathNavigation.isStableDestination(blockPos3))) {
					if (bl2) {
						blockPos3 = moveAboveSolid(blockPos3, random.nextInt(l + 1) + m, pathfinderMob.level.getMaxBuildHeight(), predicate);
					}

					if (bl || !isWaterDestination(blockPos3, pathfinderMob)) {
						BlockPathTypes blockPathTypes = WalkNodeEvaluator.getBlockPathTypeStatic(pathfinderMob.level, blockPos3.getX(), blockPos3.getY(), blockPos3.getZ());
