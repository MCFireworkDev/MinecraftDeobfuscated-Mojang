...
public class CombatTracker {
		...
		if (this.entries.isEmpty()) {
			...
			if (combatEntry != null && combatEntry2.getSource() == DamageSource.FALL) {
				...
				if (combatEntry.getSource() == DamageSource.FALL || combatEntry.getSource() == DamageSource.OUT_OF_WORLD) {
					...
					ItemStack itemStack = entity2 instanceof LivingEntity livingEntity ? livingEntity.getMainHandItem() : ItemStack.EMPTY;
	...
	public LivingEntity getKiller() {
		...
		for(CombatEntry combatEntry : this.entries) {
			...
			if (var8 instanceof Player player2 && (player == null || combatEntry.getDamage() > g)) {
				g = combatEntry.getDamage();
				player = player2;
			}

			var8 = combatEntry.getSource().getEntity();
			if (var8 instanceof LivingEntity livingEntity2 && (livingEntity == null || combatEntry.getDamage() > f)) {
