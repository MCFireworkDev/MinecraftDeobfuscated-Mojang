...
public class Scoreboard {
	...
	public Score getOrCreatePlayerScore(String string, Objective objective) {
		Map<Objective, Score> map = (Map)this.playerScores.computeIfAbsent(string, stringx -> Maps.newHashMap());
	...
	public void resetPlayerScore(String string, @Nullable Objective objective) {
		if (objective == null) {
			Map<Objective, Score> map = (Map)this.playerScores.remove(string);
			if (map != null) {
			...
			if (map != null) {
				...
				if (map.size() < 1) {
					Map<Objective, Score> map2 = (Map)this.playerScores.remove(string);
					if (map2 != null) {
						this.onPlayerRemoved(string);
					}
				} else if (score != null) {
