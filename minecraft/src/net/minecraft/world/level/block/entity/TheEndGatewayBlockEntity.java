...
public class TheEndGatewayBlockEntity extends TheEndPortalBlockEntity implements TickableBlockEntity {
	...
	private void findExitPortal(ServerLevel serverLevel) {
		...
		if (this.exitPortal == null) {
			...
			Feature.END_ISLAND
				.place(serverLevel, serverLevel.getChunkSource().getGenerator(), new Random(this.exitPortal.asLong()), this.exitPortal, FeatureConfiguration.NONE);
		} else {
	...
	private void createExitPortal(ServerLevel serverLevel, BlockPos blockPos) {
		Feature.END_GATEWAY
			.place(serverLevel, serverLevel.getChunkSource().getGenerator(), new Random(), blockPos, EndGatewayConfiguration.knownExit(this.getBlockPos(), false));
	}
