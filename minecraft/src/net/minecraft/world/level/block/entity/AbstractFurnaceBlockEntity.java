		...
		if (!this.items.get(0).isEmpty() && recipe != null) {
			...
			if (itemStack.isEmpty()) {
				...
				if (itemStack2.isEmpty()) {
					...
					return false;
				} else if (itemStack2.getCount() < this.getMaxStackSize() && itemStack2.getCount() < itemStack2.getMaxStackSize()) {
					return true;
				} else {
	...
	public void awardResetAndExperience(Player player) {
		...
		for(Entry<ResourceLocation, Integer> entry : this.recipesUsed.entrySet()) {
			player.level.getRecipeManager().byKey((ResourceLocation)entry.getKey()).ifPresent(recipe -> {
				list.add(recipe);
				createExperience(player, entry.getValue(), ((AbstractCookingRecipe)recipe).getExperience());
			});
		}
