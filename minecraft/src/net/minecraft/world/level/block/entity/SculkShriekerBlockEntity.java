...
public class SculkShriekerBlockEntity extends BlockEntity implements VibrationListener.VibrationListenerConfig {
	...
	private boolean canShriek(ServerLevel serverLevel) {
		...
		if (blockState.getValue(SculkShriekerBlock.SHRIEKING)) {
			return false;
		} else if (!blockState.getValue(SculkShriekerBlock.CAN_SUMMON)) {
			return true;
		} else {
			BlockPos blockPos = this.getBlockPos();
			return tryGetSpawnTracker(serverLevel, blockPos).map(wardenSpawnTracker -> wardenSpawnTracker.canWarn(serverLevel, blockPos)).orElse(false);
		}
	}
