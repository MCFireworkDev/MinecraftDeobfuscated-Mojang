...
public class StateDefinition<O, S extends StateHolder<S>> {
	...
	protected <A extends AbstractStateHolder<O, S>> StateDefinition(O object, StateDefinition.Factory<O, S, A> factory, Map<String, Property<?>> map) {
		...
		stream.forEach(list2 -> {
			Map<Property<?>, Comparable<?>> map2xx = MapFiller.linkedHashMapFrom(this.propertiesByName.values(), list2);
			A abstractStateHolderxx = factory.create(object, ImmutableMap.copyOf(map2xx));
			map2.put(map2xx, abstractStateHolderxx);
			list.add(abstractStateHolderxx);
		});
