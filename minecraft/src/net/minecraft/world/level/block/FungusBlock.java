...
public class FungusBlock extends BushBlock implements BonemealableBlock {
	...
	public boolean isValidBonemealTarget(LevelReader levelReader, BlockPos blockPos, BlockState blockState, boolean bl) {
		...
		if (optional.isPresent()) {
			FeatureConfiguration block = ((ConfiguredFeature)((Holder)optional.get()).value()).config();
			if (block instanceof HugeFungusConfiguration hugeFungusConfiguration) {
				Block blockx = hugeFungusConfiguration.validBaseState.getBlock();
				BlockState blockState2 = levelReader.getBlockState(blockPos.below());
				return blockState2.is(blockx);
			}
		}
