...
public class RuinedPortalFeature extends StructureFeature<RuinedPortalConfiguration> {
	...
	private static int findSuitableY(
		...
		List<BlockGetter> list2 = (List)list.stream().map(blockPos -> chunkGenerator.getBaseColumn(blockPos.getX(), blockPos.getZ())).collect(Collectors.toList());
	...
	public static class FeatureStart extends StructureStart<RuinedPortalConfiguration> {
		...
		public void generatePieces(
			...
			Rotation rotation = Util.getRandom((Rotation[])Rotation.values(), this.random);
