...
public class RuinedPortalFeature extends StructureFeature<RuinedPortalConfiguration> {
	...
	private static void generatePieces(
		...
		Rotation rotation = Util.getRandom((Rotation[])Rotation.values(), context.random());
		...
		if (context.validBiome()
			...
			structurePiecesBuilder.addPiece(
				new RuinedPortalPiece(
					context.structureManager(), blockPos4, verticalPlacement, properties, resourceLocation, structureTemplate, rotation, mirror, blockPos
				)
			);
		}
	}
