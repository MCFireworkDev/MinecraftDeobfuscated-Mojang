...
public class BonusChestFeature extends Feature<NoneFeatureConfiguration> {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> featurePlaceContext) {
		...
		List<Integer> list = Util.shuffledCopy(
			(List)IntStream.rangeClosed(chunkPos.getMinBlockX(), chunkPos.getMaxBlockX()).boxed().collect(Collectors.toList()), randomSource
		);
		List<Integer> list2 = Util.shuffledCopy(
			(List)IntStream.rangeClosed(chunkPos.getMinBlockZ(), chunkPos.getMaxBlockZ()).boxed().collect(Collectors.toList()), randomSource
		);
