							...
							if (bl && boundingBox2.getYSpan() <= 16) {
								o = list2.stream().mapToInt(structureBlockInfox -> {
									if (!boundingBox2.isInside(structureBlockInfox.pos.relative(JigsawBlock.getFrontFacing(structureBlockInfox.state)))) {
										...
										StructureTemplatePool structureTemplatePoolxx = JigsawPlacement.POOLS.getPool(resourceLocation);
										StructureTemplatePool structureTemplatePool2xx = JigsawPlacement.POOLS.getPool(structureTemplatePoolxx.getFallback());
										return Math.max(structureTemplatePoolxx.getMaxSize(this.structureManager), structureTemplatePool2xx.getMaxSize(this.structureManager));
									}
								}).max().orElse(0);
							} else {
								o = 0;
