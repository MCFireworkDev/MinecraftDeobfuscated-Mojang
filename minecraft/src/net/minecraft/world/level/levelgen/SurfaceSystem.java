...
public class SurfaceSystem {
	...
	public void buildSurface(
		...
		for(int k = 0; k < 16; ++k) {
			for(int l = 0; l < 16; ++l) {
				...
				ResourceKey<Biome> resourceKey = (ResourceKey)registry.getResourceKey(biome).orElseThrow(() -> new IllegalStateException("Unregistered biome: " + biome));
				...
				for(int aa = t; aa >= u && x < v; --aa) {
					...
					if (blockState.isAir()) {
						...
						ResourceKey<Biome> resourceKey2 = (ResourceKey)registry.getResourceKey(biome2)
	...
	private static BlockState[] generateBands(RandomSource randomSource) {
		...
		int i = randomSource.nextIntBetweenInclusive(9, 15);
		int j = 0;

		for(int k = 0; j < i && k < blockStates.length; k += randomSource.nextInt(16) + 4) {
