		...
		if (registry.size() != BUILTIN_ORDER.size()) {
			...
			if (!optional.isEmpty() && !optional2.isEmpty() && !optional3.isEmpty()) {
				if (!((LevelStem)optional.get()).typeHolder().value().equalTo(DimensionType.DEFAULT_OVERWORLD)
					...
					return false;
				} else if (((LevelStem)optional2.get()).generator() instanceof NoiseBasedChunkGenerator noiseBasedChunkGenerator
					&& ((LevelStem)optional3.get()).generator() instanceof NoiseBasedChunkGenerator noiseBasedChunkGenerator2) {
