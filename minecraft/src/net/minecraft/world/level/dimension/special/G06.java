	...
	public static class ShapesBiome extends Biome {
		public ShapesBiome() {
				...
				.forEach(
					block -> {
						...
						list.add(
							Feature.SHAPE
								.configured(
									new ShapeConfiguration(new SimpleStateProvider(block.defaultBlockState()), Util.randomObject(random, ShapeConfiguration.Metric.values()), fxx, gxx)
								)
						);
					}
				);
			...
			for(Block[] blocks : ColoredBlocks.COLORED_BLOCKS) {
				ImmutableList<BlockState> immutableList = (ImmutableList)Stream.of(blocks).map(Block::defaultBlockState).collect(ImmutableList.toImmutableList());
