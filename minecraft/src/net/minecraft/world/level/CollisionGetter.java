		...
		return StreamSupport.stream(new AbstractSpliterator<VoxelShape>(Long.MAX_VALUE, 1280) {
			...
			public boolean tryAdvance(Consumer<? super VoxelShape> consumer) {
				if (!this.checkedBorder) {
					...
					VoxelShape voxelShapex = CollisionGetter.this.getWorldBorder().getCollisionShape();
					boolean bl = Shapes.joinIsNotEmpty(voxelShapex, Shapes.create(entity.getBoundingBox().deflate(1.0E-7)), BooleanOp.AND);
					boolean bl2 = Shapes.joinIsNotEmpty(voxelShapex, Shapes.create(entity.getBoundingBox().inflate(1.0E-7)), BooleanOp.AND);
					if (!bl && bl2) {
