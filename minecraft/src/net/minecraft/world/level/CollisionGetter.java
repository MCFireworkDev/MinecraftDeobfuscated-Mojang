					...
					if (l != 3) {
						...
						if (blockGetter != null) {
							...
							if ((l != 1 || blockState.hasLargeCollisionShape()) && (l != 2 || blockState.is(Blocks.MOVING_PISTON))) {
								VoxelShape voxelShapex = blockState.getCollisionShape(CollisionGetter.this, mutableBlockPos, collisionContext);
								if (voxelShapex == Shapes.block()) {
									if (aABB.intersects((double)i, (double)j, (double)k, (double)i + 1.0, (double)j + 1.0, (double)k + 1.0)) {
										consumer.accept(voxelShapex.move((double)i, (double)j, (double)k));
										return true;
									}
								} else {
									VoxelShape voxelShape2 = voxelShapex.move((double)i, (double)j, (double)k);
