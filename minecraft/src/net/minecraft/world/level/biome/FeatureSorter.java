...
public class FeatureSorter {
	public static <T> List<FeatureSorter.StepFeatureData> buildFeaturesPerStep(List<T> list, Function<T, List<HolderSet<PlacedFeature>>> function, boolean bl) {
		...
		for(T object : list) {
			...
			List<HolderSet<PlacedFeature>> list3 = (List)function.apply(object);
		...
		for(FeatureData lv : map.keySet()) {
			...
			if (!set2.contains(lv) && Graph.depthFirstSearch(map, set2, set3, list2::add, (T)lv)) {
				if (!bl) {
		...
		for(int j = 0; j < i; ++j) {
			int l = j;
			List<PlacedFeature> list5 = (List)list2.stream().filter(arg -> arg.step() == l).map(FeatureData::feature).collect(Collectors.toList());
