...
public class BiomeDefaultFeatures {
	...
	public static void addSwampVegetation(Biome biome) {
		biome.addFeature(
			GenerationStep.Decoration.VEGETAL_DECORATION,
			Biome.makeComposite(
				Feature.SWAMP_TREE, FeatureConfiguration.NONE, FeatureDecorator.COUNT_EXTRA_HEIGHTMAP, new DecoratorFrequencyWithExtraChance(2, 0.1F, 1)
			)
		);
		biome.addFeature(
			GenerationStep.Decoration.VEGETAL_DECORATION,
	...
	public static void addDefaultSprings(Biome biome) {
		...
		biome.addFeature(
			...
			Biome.makeComposite(
				Feature.SPRING,
				new SpringConfiguration(Fluids.LAVA.defaultFluidState()),
				FeatureDecorator.COUNT_VERY_BIASED_RANGE,
				new DecoratorCountRange(20, 8, 16, 256)
			)
		);
	}
