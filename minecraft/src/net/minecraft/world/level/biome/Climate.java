...
public class Climate {
	...
	public static class ParameterList<T> {
		...
		public T findValueBruteForce(Climate.TargetPoint targetPoint) {
			...
			long l = ((Climate.ParameterPoint)pair.getFirst()).fitness(targetPoint);
			T object = pair.getSecond();

			while(iterator.hasNext()) {
				Pair<Climate.ParameterPoint, T> pair2 = (Pair)iterator.next();
				long m = ((Climate.ParameterPoint)pair2.getFirst()).fitness(targetPoint);
				if (m < l) {
					l = m;
	...
	protected static final class RTree<T> {
		...
		public static <T> Climate.RTree<T> create(List<Pair<Climate.ParameterPoint, T>> list) {
			if (list.isEmpty()) {
				...
				if (i != 7) {
					...
					List<Climate.RTree.Leaf<T>> list2 = (List)list.stream()
			...
			if (list.isEmpty()) {
				...
				list.sort(Comparator.comparingLong(node -> {
					...
					for(int jxx = 0; jxx < i; ++jxx) {
						Climate.Parameter parameter = node.parameterSpace[jxx];
						lxx += Math.abs((parameter.min() + parameter.max()) / 2L);
					}
		...
		static <T> List<Climate.Parameter> buildParameterSpace(List<? extends Climate.RTree.Node<T>> list) {
			if (list.isEmpty()) {
				...
				int i = 7;
				List<Climate.Parameter> list2 = Lists.newArrayList();
		...
		static final class SubTree<T> extends Climate.RTree.Node<T> {
			...
			protected SubTree(List<Climate.Parameter> list, List<? extends Climate.RTree.Node<T>> list2) {
				super(list);
				this.children = (Climate.RTree.Node[])list2.toArray(new Climate.RTree.Node[0]);
			}
