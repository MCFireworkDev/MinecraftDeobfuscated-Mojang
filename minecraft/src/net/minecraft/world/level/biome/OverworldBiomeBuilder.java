...
public final class OverworldBiomeBuilder {
	...
	private void addPeaks(Builder<Pair<Climate.ParameterPoint, ResourceKey<Biome>>> builder, Climate.Parameter parameter) {
		...
		if (parameter.max() < 0.0F) {
			...
			this.addSurfaceBiome(
				builder,
				Climate.range(this.temperatures[0], this.temperatures[2]),
				this.FULL_RANGE,
				parameter3,
				this.erosions[0],
				parameter,
				0.0F,
				Biomes.SNOWCAPPED_PEAKS
			);
		}
	...
	private void addUndergroundBiomes(Builder<Pair<Climate.ParameterPoint, ResourceKey<Biome>>> builder) {
		this.addUndergroundBiome(
			builder, this.FULL_RANGE, this.FULL_RANGE, Climate.range(0.8F, 1.0F), this.FULL_RANGE, this.FULL_RANGE, 0.0F, Biomes.DRIPSTONE_CAVES
		);
	...
	private void addDebugBiomesToShowCoastalAreas(Builder<Pair<Climate.ParameterPoint, ResourceKey<Biome>>> builder) {
		...
		this.addSurfaceBiome(
			builder, this.FULL_RANGE, this.FULL_RANGE, Climate.range(-0.05F, 1.0F), this.FULL_RANGE, Climate.range(0.15F, 1.0F), 0.0F, Biomes.PLAINS
		);
	}
}
