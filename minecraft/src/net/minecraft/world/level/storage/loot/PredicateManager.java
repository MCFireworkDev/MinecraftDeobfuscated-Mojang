		...
		map.forEach((resourceLocation, jsonElement) -> {
			try {
				if (jsonElement.isJsonArray()) {
					LootItemCondition[] lootItemConditions = (LootItemCondition[])GSON.fromJson(jsonElement, LootItemCondition[].class);
				...
				LOGGER.error("Couldn't parse loot table {}", resourceLocation, var4xx);
			}
		});
