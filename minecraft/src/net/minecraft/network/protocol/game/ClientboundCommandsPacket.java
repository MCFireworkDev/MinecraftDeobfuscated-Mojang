...
public class ClientboundCommandsPacket implements Packet<ClientGamePacketListener> {
	...
	public void write(FriendlyByteBuf friendlyByteBuf) throws IOException {
		Map<CommandNode<SharedSuggestionProvider>, Integer> map = Maps.newHashMap();
		Deque<CommandNode<SharedSuggestionProvider>> deque = new ArrayDeque();
		deque.add(this.root);

		while(!deque.isEmpty()) {
			CommandNode<SharedSuggestionProvider> commandNode = (CommandNode)deque.pollFirst();
		...
		friendlyByteBuf.writeVarInt(map.get(this.root));
	}
