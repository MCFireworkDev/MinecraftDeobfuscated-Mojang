		...
		if (j == 0) {
			...
			for(int l = 1; l < j; ++l) {
				...
				if (!style2.equals(style)) {
					...
					list.add(
						bl ^ style2.isReversed() ? FormattedCharSequence.backward(string, style, this.reverseCharModifier) : FormattedCharSequence.forward(string, style)
					);
					style = style2;
					k = m;
				}
			...
			if (k < i + j) {
				...
				list.add(
					bl ^ style.isReversed() ? FormattedCharSequence.backward(string2, style, this.reverseCharModifier) : FormattedCharSequence.forward(string2, style)
				);
			}
