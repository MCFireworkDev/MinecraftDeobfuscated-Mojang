...
public abstract class RecipeProvider implements DataProvider {
	...
	protected static void stonecutterResultFromBase(
		Consumer<FinishedRecipe> consumer, RecipeCategory recipeCategory, ItemLike itemLike, ItemLike itemLike2, int i
	) {
	...
	private static InventoryChangeTrigger.TriggerInstance inventoryTrigger(ItemPredicate.Builder... builders) {
		return inventoryTrigger((ItemPredicate[])Arrays.stream(builders).flatMap(builder -> builder.build().stream()).toArray(i -> new ItemPredicate[i]));
	}
