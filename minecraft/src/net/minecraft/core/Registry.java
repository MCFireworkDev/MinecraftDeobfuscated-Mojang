	...
	public <U> DataResult<Pair<T, U>> decode(DynamicOps<U> dynamicOps, U object) {
		return dynamicOps.compressMaps()
			? dynamicOps.getNumberValue(object).flatMap(number -> {
				T objectxx = this.byId(number.intValue());
				return objectxx == null ? DataResult.error("Unknown registry id in " + this.key + ": " + number) : DataResult.success(objectxx, this.lifecycle(objectxx));
			}).map(objectx -> Pair.of(objectx, dynamicOps.empty()))
			: ResourceLocation.CODEC
				.decode(dynamicOps, object)
				.flatMap(
					pair -> {
						T objectxx = this.get((ResourceLocation)pair.getFirst());
						return objectxx == null
							? DataResult.error("Unknown registry key in " + this.key + ": " + pair.getFirst())
							: DataResult.success(Pair.of(objectxx, pair.getSecond()), this.lifecycle(objectxx));
					}
				);
	}
