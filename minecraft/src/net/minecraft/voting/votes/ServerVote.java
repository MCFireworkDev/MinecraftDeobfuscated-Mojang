...
public record ServerVote(CommonVoteData header, Map<OptionId, ServerVote.Option> options) {
	...
	public static Optional<ServerVote> createRandomApproveVote(
		...
		rule.randomApprovableChanges(minecraftServer, voteGenerationOptions.random, i).forEach(ruleChange -> {
			List<RuleChange> list2xx = new ArrayList();
			list2xx.add(ruleChange);
			list.add(list2xx);
		});
		if (list.isEmpty()) {
			...
			List<ServerVote.Option> list2 = list.stream().map(listx -> {
				List<ServerVote.Effect> list2xx = listx.stream().map(ruleChange -> new ServerVote.Effect(ruleChange, ruleAction)).toList();
				return new ServerVote.Option(getRuleDisplayName(list2xx), list2xx);
			}).toList();
	...
	public static Optional<ServerVote> createRandomRepealVote(
		...
		List<ServerVote.Option> list = (List)randomUnweightedVoteStream(set, voteGenerationOptions)
			.distinct()
			.flatMap(reference -> ((Rule)reference.value()).repealableChanges())
			.limit((long)i)
			.map(ruleChange -> {
				List<ServerVote.Effect> listxx = List.of(new ServerVote.Effect(ruleChange, RuleAction.REPEAL));
				return new ServerVote.Option(getRuleDisplayName(listxx), listxx);
			})
	...
	public static record Option(Component displayName, List<ServerVote.Effect> changes) {
		final List<ServerVote.Effect> changes;
	...
	public static record VoteGenerationOptions(
		...
		final List<VotingMaterial.Cost> voteCost;
		...
		public static ServerVote.VoteGenerationOptions createFromRules(RandomSource randomSource) {
			return new ServerVote.VoteGenerationOptions(
				...
				Rules.NEW_VOTE_EXTRA_EFFECT_MAX_COUNT.get(),
				Rules.NEW_VOTE_COST.get(),
				!Rules.NEW_VOTE_NO_OPT_OUT.get(),
				Rules.NEW_VOTE_MAX_APPROVE_VOTE_COUNT.get(),
				Rules.NEW_VOTE_MAX_REPEAL_VOTE_COUNT.get(),
