...
public class SoundEventReplacementRule extends ResourceKeyReplacementRule<SoundEvent> {
	...
	public SoundEvent replace(SoundEvent soundEvent) {
		return (SoundEvent)this.cache.computeIfAbsent(soundEvent, soundEvent2 -> {
			ResourceKey<SoundEvent> resourceKey = (ResourceKey)BuiltInRegistries.SOUND_EVENT.getResourceKey(soundEvent2).orElse(null);
			if (resourceKey != null) {
				ResourceKey<SoundEvent> resourceKey2 = (ResourceKey)this.entries.get(resourceKey);
