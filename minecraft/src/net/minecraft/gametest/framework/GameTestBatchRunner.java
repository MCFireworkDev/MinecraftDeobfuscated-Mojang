...
public class GameTestBatchRunner {
	...
	public GameTestBatchRunner(
		...
		collection.forEach(gameTestBatch -> {
			Collection<GameTestInfo> collectionxx = Lists.<GameTestInfo>newArrayList();
	...
	private void runBatch(int i) {
		...
		if (i < this.batches.size()) {
			Pair<GameTestBatch, Collection<GameTestInfo>> pair = (Pair)this.batches.get(this.currentBatchIndex);
