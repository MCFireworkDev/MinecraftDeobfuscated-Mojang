...
public class RealmsPlayerScreen extends RealmsScreen {
	...
	private void uninvite(int i) {
		...
		if (i >= 0 && i < this.serverData.players.size()) {
			...
			}, new TextComponent("Question"), new TranslatableComponent("mco.configure.world.uninvite.question")
				.append(" '")
	...
	class Entry extends ObjectSelectionList.Entry<RealmsPlayerScreen.Entry> {
		...
		private void renderInvitedItem(PoseStack poseStack, PlayerInfo playerInfo, int i, int j, int k, int l) {
			...
			RealmsPlayerScreen.this.font
				.draw(
					poseStack, I18n.get("mco.configure.world.activityfeed.disabled"), (float)RealmsPlayerScreen.this.column2X, (float)RealmsPlayerScreen.row(5), 10526880
				);
